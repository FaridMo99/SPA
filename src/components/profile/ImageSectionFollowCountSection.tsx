...
function ImageSectionFollowCountSection({ followers, following, username }: ImageSectioFollowCountSectionProps) {
  ...
  return (
    <div className="font-bold text-green-300 dark:text-dark-green flex justify-evenly items-center absolute top-9 left-22 w-1/2">
      ...
      {isFollowersOpen && <FollowersModal username={username} setIsOpen={setIsFollowersOpen}/>}
      {isFollowingOpen && <FollowingsModal username={username} setIsOpen={setIsFollowingOpen}/>}
    </div>
  );
}
