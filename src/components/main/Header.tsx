import type { Avatar } from "../../types/types";
import UserImage from "../ui/UserImage";
import Searchbar from "./Searchbar";
import { Link } from "react-router-dom";



function Header({ avatar }: { avatar: Avatar}) {
  return (
    <header className="w-full h-[15vh] border-green-300 dark:border-dark-green border-b-2 flex items-center justify-between fixed top-0 right-0 bg-gray-50 dark:bg-dark-gray z-498">
      <div className="w-full h-full flex items-center">
        <Link
          to="/home"
          className="font-bold text-4xl self-start ml-4 mt-4 w-1/4"
        >
          <h1>friendly.</h1>
        </Link>
        <Searchbar />
      </div>
      <div className="md:w-[20vw] w-[30vw] h-full border-l-2 border-green-300 dark:border-dark-green flex justify-center items-center">
        <Link to="/profile">
          <UserImage img={avatar} />
        </Link>
      </div>
    </header>
  );
}

export default Header;
